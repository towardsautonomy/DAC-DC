...
def DAC_DC(shape=(256,256,3)):
    ...
    drop6 = Dropout(rate=0.)(pool6)

    conv_final = Convolution2D(filters=512, kernel_size=(1, 1), padding = 'same', kernel_initializer = 'glorot_uniform')(drop6)

    # FC Layers
    output_size = n_x_grids*n_y_grids*n_anchors*n_elements_per_anchor
...
def DAC_DC_with_class(shape=(320,320,3)):
    output_size = n_x_grids*n_y_grids*n_anchors*n_elements_per_anchor
    inputs = Input(shape)
    ...
    # Block 7
    layer7 = Convolution2D(filters=1024, kernel_size=(3, 3), padding = 'same', kernel_initializer = 'glorot_uniform')(pool6)
    layer7 = BatchNormalization()(layer7)
    layer7 = LeakyReLU(alpha=0.1)(layer7)
    layer7 = Convolution2D(filters=1024, kernel_size=(3, 3), padding = 'same', kernel_initializer = 'glorot_uniform')(layer7)
    layer7 = BatchNormalization()(layer7)
    layer7 = LeakyReLU(alpha=0.1)(layer7)
    merge7 = Concatenate(axis = 3)([layer7,pool6])
    pool7 = MaxPooling2D(pool_size=(2, 2))(merge7)

    conv_final = Convolution2D(filters=output_size, kernel_size=(1, 1), padding = 'same', kernel_initializer = 'glorot_uniform')(pool7)
    conv_final = MaxPooling2D(pool_size=(2, 2))(conv_final)
    conv_final = Convolution2D(filters=output_size, kernel_size=(1, 1), padding = 'same', kernel_initializer = 'glorot_uniform')(conv_final)
    conv_final = Activation('sigmoid')(conv_final)

    # flatten for output layer
    out = Flatten()(conv_final)
